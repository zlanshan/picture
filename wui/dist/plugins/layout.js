var Layout = function () {
    ...
    var handleSidebarMenu = function () {
        ...
        $('.page-sidebar-menu').on('click', 'li > a, li > a > span.nav-toggle', function (e) {
            ...
            if (hasSubMenu === false) {
                ...
                var openli =  $('.page-sidebar-menu').find('li.open');
                for(var i=0; i< openli.length; i++){
                    if(openli.eq(i).find('.sub-menu').length <= 0){
                        openli.eq(i).removeClass('open')                    }
                };
                that.parent().siblings().removeClass('open');
            ...
            var slideSpeed = parseInt(menu.data("slide-speed"));
            var keepExpand = menu.data("keep-expanded");
            var closeOthers = menu.data("close-others");
            
            if (!keepExpand) {
                parent.children('li.open').children('a').children('.arrow').removeClass('open');
                parent.children('li.open').children('.sub-menu:not(.always-open)').slideUp(slideSpeed);
                parent.children('li.open').removeClass('open');
            }else if(closeOthers && !$(this).closest('.nav-item').hasClass("open")){
                parent.children('li.open').children('a').children('.arrow').removeClass('open');
                parent.children('li.open').children('.sub-menu:not(.always-open)').slideUp(slideSpeed);
                parent.children('li.open').removeClass('open');
            }
