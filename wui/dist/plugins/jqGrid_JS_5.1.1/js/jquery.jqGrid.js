...
$.extend($.jgrid,{
	...
	parseDate : function(format, date, newformat, opts) {
		...
		if(date && date != null) {
			...
			/* 
			 * update by feily 2017-01-04 去掉格式 u验证
			...
			// end update by feily 2017-01-04 
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			/*
			 * 添加设置默认列宽度参数
			 * @defaultColWidth {Number} 默认值是150
			 */
			defaultColWidth: 150,
			scrollPopUp : false,
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='"+btc+"' title='"+ pot[0] +"'" + "><span " + getstyle(stylemodule, 'icon_first', false, iconbase)+">"+ pot[0] +"</span></td>";
				pgl += "<td id='"+po[1]+"' class='"+btc+"'  title='"+ pot[1] +"'" +"><span " + getstyle(stylemodule, 'icon_prev', false, iconbase)+">"+ pot[1] +"</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='"+btc+"' title='"+ pot[2] +"'" +"><span " + getstyle(stylemodule, 'icon_next',false, iconbase)+">"+ pot[2] +"</span></td>";
				pgl += "<td id='"+po[3]+"' class='"+btc+"' title='"+ pot[3] +"'" +"><span " + getstyle(stylemodule, 'icon_end',false, iconbase)+">"+ pot[3] +"</span></td>";
			} else if (pginp !== "") {
				pgl += pginp;
			}
			if(dir==="ltr") {
		...
		multiSort = function(iCol, obj, sor ) {
			...
			$("span[sort]", selTh).removeClass("wui-text-primary");
			...
			if( so ) {
				...
				// 修改排序图标
				// chenzx
				$("span[sort='"+ so +"']", selTh).removeClass(disabled);
				$("span[sort='"+ so +"']", selTh).addClass("wui-text-primary");
		...
		buildColMenu = function( index, left, top ){
			...
			if(op.columns) {
				...
				str += '<li class="ui-menu-item" role="presentation"><a class="g-menu-item" tabindex="0" role="menuitem" value="columns"><table class="ui-common-table"><tr><td class="menu_icon"><span class="'+iconbase+' '+colmenustyle.icon_columns+'"></span></td><td class="menu_text">筛选菜单</td></tr></table></a></li>';
			}
			if(op.filtering) {
				str += '<li class="ui-menu-item divider" role="separator"></li>';
		...
		// 修改排序图标
		// chenzx
		imgs = "<span class='s-ico'>";
		imgs += "<span sort='asc' title='正序排序' class='ui-grid-ico-sort "+iac+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_asc', true)+ "'" + sort + "></span>";
		imgs += "<span sort='desc' title='倒序排序' class='ui-grid-ico-sort "+idc+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_desc', true)+"'" + sort + "></span></span>";
		if(ts.p.multiSort) {
			if(ts.p.sortname ) {
		...
		$("th",thr).each(function ( j ) {
			...
			if(tmpcm.resizable){
				...
				$(res).addClass('ui-jqgrid-resize ui-jqgrid-resize-'+dir)
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("div.ui-jqgrid-sortable").each(function () {
					...
					if ($parent.is(":visible") && $parent.is(":has(span.ui-jqgrid-resize)")) {
						...
						// console.group();
						// console.error($parent.height())
						// console.error($ts.outerHeight())
						// console.error(($parent.height() - $ts.outerHeight()) / 2  - 4)
						// console.groupEnd();
